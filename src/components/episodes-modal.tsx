...
export function EpisodesModal({
  ...
  const isCurrentEpisode = (ep: any) => ep.id === currentEpisodeId;
        ...
        >
          <div className="sticky top-0 flex items-center justify-between border-b border-white/[0.08] bg-charcoal-raised/95 p-4 backdrop-blur-xl z-10">
            <div className="flex items-center gap-3">
              <button
                onClick={() => onOpenChange(false)}
                className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.1] bg-white/[0.04] text-muted-foreground transition-colors hover:border-white/20 hover:bg-white/10"
                aria-label="Close"
              >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Season {selectedSeason} · {series.seasons.length} seasons
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-h-[65vh] overflow-y-auto p-4">
            <div className="space-y-2">
              {episodes.map((ep) => {
                const unlocked = ep.monetization === "free"; // Simplified for now
              ...
              {episodes.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <svg className="h-10 w-10 text-muted-foreground/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    <path d="M10 2v20M2 10h20" strokeWidth={1.5} />
                  </svg>
